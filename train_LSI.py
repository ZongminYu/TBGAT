...
class NeuralTabu:
    def __init__(self):
        ...
        # generate
            ...
            self.logger.info('No validation data for {}-{}x{}[{},{}], generating new one.'.format(args.problem_type, args.j, args.m, args.l, args.h))
            ...
            self.logger.info('Solving validation data using CP-SAT with 3600s limit...')
    ...
    def validation(self, policy, dev, batch_i):
        ...
        if gap_incumbent < self.gap_incumbent:
            ...
            #     )
            # )
            self.logger.info("Saving trained_model in batch_i :{}".format(batch_i))
            torch.save(
                policy.state_dict(), 
                '{}/Dv-{}-checkpoint-{}.pt'.format(
                    self.result_folder, self.algo_config, batch_i
                )
            )
    ...
    def train(self):
        ...
        self.logger.info("Use {} to train...".format(dev))
        self.logger.info("{}, "
              "{}x{}, "
              "init_type={}, "
              "lr={}, "
        ...
        for batch_i in pbar:
            ...
            if batch_i % args.step_validation == 0:
                gap_incumbent, gap_last_step = self.validation(policy, dev, batch_i)
