...
class Env:
    ...
    def step(self, action=None, ns_type='N5', prt=False, show_action_space_compute_time=False):
        ...
        if ns_type == 'N5':
            ...
            action_instance_id = self.G_batch.batch[u]
...
if __name__ == '__main__':
    ...
    # self.logger.info(env.current_objs)
    # self.logger.info(env.num_nodes_per_example - 2)

    t1 = time.time()
    for _ in range(30):
        self.logger.info('step {}'.format(_))
        env.step(prt=print_step_time, show_action_space_compute_time=print_action_space_compute_time)
        # self.logger.info(env.current_objs)
        # self.logger.info(env.incumbent_objs)
        self.logger.info()
    t2 = time.time()
    # self.logger.info(t2 - t1)
    env.cpm_eval()

    # self.logger.info(env.get_candidate_moves(prt=False)[0])
    ...
    self.logger.info(time.time() - ttt)
    ...
    for i, (s, t) in enumerate(zip(env.S, env.T)):
        ...
        if sorted(neural_path) != sorted(paths):
            self.logger.info('NO!!!!!!!!!!!!!!!!!!!!!!')
            self.logger.info(sorted(neural_path))
            self.logger.info(sorted(paths))
    self.logger.info(time.time() - tt)
    ...
    for _, inst in enumerate(insts):
        self.logger.info('Processing instance:', _ + 1)
    ...
    self.logger.info(indeed_optimal)
    self.logger.info(should_be_optimal)
    if np.isin(should_be_optimal, indeed_optimal).sum() == should_be_optimal.shape[0]:
        self.logger.info("Pass N5 property check (property in page 6 of the paper).")
    else:
        self.logger.info("!!!Do not pass N5 property check (property in page 6 of the paper).")
